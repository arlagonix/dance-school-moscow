...
const FadeInWrapper = ({ children, delay = 0 }: ComponentProps) => {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20, scale: 0 }} // start with opacity 0 and slightly shifted down
      ...
      viewport={{ once: true, amount: 0.5 }} // trigger animation once when 50% of the component is visible
    >
      {children}
    </motion.div>
  )
